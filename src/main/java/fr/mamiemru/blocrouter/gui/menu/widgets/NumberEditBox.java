...
@OnlyIn(Dist.CLIENT)
public class NumberEditBox extends AbstractWidget implements Widget, GuiEventListener {
    ...
    private static final int BORDER_COLOR_FOCUSED = -16777216;
    ...
    private Predicate<String> filter = (x) -> Objects.nonNull(x) && !x.isEmpty() && StringUtils.isNumeric(x);
    ...
    public NumberEditBox(Font pFont, int pX, int pY, int pWidth, int pHeight, Component pMessage) {
        this(pFont, pX, pY, pWidth, pHeight, (NumberEditBox)null, pMessage);
        setMaxLength(5);
    }

    public NumberEditBox(Font pFont, int pX, int pY, int pWidth, int pHeight, @Nullable NumberEditBox p_94111_, Component pMessage) {
     ...
     */
    public void setValue(int pText) {
        setValue(String.valueOf(pText));
    }
     ...
     */
    public int getValue() {
        return (this.filter.test(this.value)) ? Integer.valueOf(this.value) : 0;
    }
